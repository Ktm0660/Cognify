...
const ITEMS = [
  ...
  },
  {
    ...
    options: [
      ...
      {
        id: "b",
        label: "Test a new rule (e.g., +2, then +3), and see if it fits.",
      },
  ...
  },
  {
    ...
    options: [
      { id: "a", label: "Lock your first instinct quickly to save time." },
      {
        id: "b",
        label:
          "Pause and test one assumption that could falsify your choice.",
      },
...
function scoreAssessment(ans: Answer[]): TraitScores {
  const base: TraitScores = {
    exploration: 0, logic: 0, adaptability: 0,
    risk: 0, cooperation: 0, metacognition: 0, creativity: 0
  };
  for (const a of ans) {
...
export default function MiniAssessmentPage() {
  ...
  if (saving) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-indigo-50 to-white dark:from-slate-950 dark:to-slate-900">
        <div className="container-page py-20">
          <div className="card p-8 text-center">
            <h2 className="text-2xl font-bold">Saving your Thinking Snapshot…</h2>
            <p className="mt-2 text-slate-600 dark:text-slate-300">Redirecting to home.</p>
          </div>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-gradient-to-b from-indigo-50 to-white dark:from-slate-950 dark:to-slate-900">
      ...
      <section className="container-page py-8">
        <div className="card p-6 sm:p-8">
          <p className="text-sm text-indigo-600 font-semibold">{LABEL[item.trait]}</p>
          <h1 className="mt-1 text-2xl sm:text-3xl font-extrabold">
            {LABEL[item.trait]} • Question
          </h1>

          <p className="mt-4 text-slate-700 dark:text-slate-300">{item.prompt}</p>
          {item.note && (
          ...
          <div className="mt-6 grid gap-3">
            {item.options.map((op) => (
              <button
                ...
                className="w-full text-left rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 px-4 py-3 hover:bg-indigo-50 dark:hover:bg-slate-800 transition"
              >
                {op.label}
              </button>
            ))}
          ...
          {item["askConfidence"] && (
            <div className="mt-6 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-4">
              <p className="text-sm text-slate-600 dark:text-slate-300">How confident are you? (1–5)</p>
              <Confidence onPick={setConf} />
            </div>
          )}
...
function Confidence({ onPick }: { onPick: (c: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-3">
        <span className="text-xs text-slate-500 dark:text-slate-400">Confidence: {val}/5</span>
        <button
          className="ml-auto btn-ghost text-sm px-3 py-1"
          onClick={() => onPick(val)}
        >
